...
- 兩個來源播放器保持靜音並置於可見視窗；以選定音軌的 AudioBufferSourceNode 和 AudioContext 時鐘控制輸出，影片播放落後會 seek 校正，不再一超過 0.25 秒就中止。持續 8 秒無法恢復才回報錯誤。
- 保留前端裁切；音軌使用相同裁切起點，較短影片结束後黑幕，聲音結束後靜音。可選輸出標準 720p 或省電 480p；此設定降低合成與編碼負擔，不改變來源檔的解碼解析度。
- 每個播放、解碼、定位、封裝等待均有逾時與取消處理。26 項自動化測試通過，包含模擬播放器落後 3 秒恢復、原始偏移 4 秒裁切與輸出生命週期。這些模拟測試不等於 iPhone 真機編碼驗證。
