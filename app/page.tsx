...
export default function Home() {
  ...
  return (
    <main>
      ...
      <div className="intro">
        <div>
          <p className="eyebrow">TWO VIEWS. ONE MOMENT.</p>
          <h1>
            把兩個視角，<em>放在一起。</em>
          </h1>
        </div>
          ...
          <div className="sources">
            {clips.map((clip, i) => (
              <div key={i} className="source-item">
                <label
                  className={`upload ${clip ? 'loaded' : ''} ${locked ? 'locked' : ''}`}
                >
        ...
        <section className="editor">
          ...
          {busy.includes('融合') && (
            <div className="progress-panel">
              ...
              <p>輸出時間約等於影片長度。請保持 Safari 在前景，勿鎖定螢幕。</p>
            </div>
          )}
        </section>
      </div>
      {result && (
        <section className="result-panel">
          ...
          <p className="hint">
            iPhone：點分享，再選「儲存影片」。若沒有此選項，先下載到「檔案」後再開啟分享。
            {!result.blob.type.includes('mp4') &&
              ' 本瀏覽器輸出 WebM；要存入照片，請改用支援 MP4 的 Safari。'}
          </p>
        </section>
      )}
      <dialog
